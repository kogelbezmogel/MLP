...
Tensor Model::operator() (SimpleTensor input) {
    Tensor x;
...
Layer* Model::operator[] (std::string layer_name) { return _layer_map[layer_name]; }

std::vector<std::string> Model::getLayers() {
...
SimpleTensor Model::predict(SimpleTensor input) {
    Tensor x(input);
    for(Layer* layer : _layer_sequence) {
        x = layer -> operator()(x);
    }
...
void Model::setLossFun(std::string loss_name) {
...
std::map<std::string, float> Model::gradient_report() {
...
Layer* Model::getLayer(std::string name) { return _layer_map[name]; }
